...
/// Iterates over cards matching a tag mask
pub const TagIterator = struct {
    items: []*Instance,
    mask: cards.TagSet,
    index: usize,

    pub fn init(items: []*Instance, mask: cards.TagSet) TagIterator {
        return .{ .items = items, .mask = mask, .index = 0 };
    }

    pub fn next(self: *TagIterator) ?*Instance {
        while (self.index < self.items.len) {
            const card = self.items[self.index];
            self.index += 1;
            if (card.template.tags.hasAnyTag(self.mask)) {
                return card;
            }
        }
        return null;
    ...
    pub fn reset(self: *TagIterator) void {
        self.index = 0;
    }
};
...
pub const Deck = struct {
    ...
    /// Count cards in draw pile matching any tag in mask
    pub fn countByTag(self: *const Deck, tag_mask: cards.TagSet) usize {
        var count: usize = 0;
        for (self.draw.items) |card| {
            if (card.template.tags.hasAnyTag(tag_mask)) {
                count += 1;
            }
        }
        return count;
    ...
    /// Iterate cards in draw pile matching any tag in mask
    pub fn drawableByTag(self: *Deck, tag_mask: cards.TagSet) TagIterator {
...
test "countByTag returns zero for empty draw pile" {
    var deck = try Deck.init(testing.allocator, &card_list.BeginnerDeck);
    defer deck.deinit();

    // All cards start in discard, draw is empty
    try testing.expectEqual(@as(usize, 0), deck.countByTag(.{ .offensive = true }));
    try testing.expectEqual(@as(usize, 0), deck.countByTag(.{ .defensive = true }));
}

test "countByTag counts offensive cards in draw pile" {
    var deck = try Deck.init(testing.allocator, &card_list.BeginnerDeck);
    defer deck.deinit();

    // Move all cards from discard to draw
    while (deck.discard.items.len > 0) {
        const card = deck.discard.pop().?;
        try deck.draw.append(deck.alloc, card);
    }

    // BeginnerDeck: thrust(5), slash(5), shield_block(5), riposte(5) = 20 total
    // Offensive: thrust, slash, riposte = 15
    const offensive_count = deck.countByTag(.{ .offensive = true });
    try testing.expectEqual(@as(usize, 15), offensive_count);

    // Defensive: shield_block = 5
    const defensive_count = deck.countByTag(.{ .defensive = true });
    try testing.expectEqual(@as(usize, 5), defensive_count);

    // No manoeuvre cards yet
    const manoeuvre_count = deck.countByTag(.{ .manoeuvre = true });
    try testing.expectEqual(@as(usize, 0), manoeuvre_count);
}

test "drawableByTag iterates matching cards" {
    var deck = try Deck.init(testing.allocator, &card_list.BeginnerDeck);
    defer deck.deinit();
    ...
    // Count offensive cards via iterator
    var iter = deck.drawableByTag(.{ .offensive = true });
    var count: usize = 0;
    while (iter.next()) |_| {
        count += 1;
    }
    ...
    iter.reset();
    count = 0;
    while (iter.next()) |card| {
        try testing.expect(card.template.tags.offensive);
        count += 1;
    }
    try testing.expectEqual(@as(usize, 15), count);
}

test "drawableByTag returns empty iterator for no matches" {
    var deck = try Deck.init(testing.allocator, &card_list.BeginnerDeck);
    defer deck.deinit();
    ...
    // No manoeuvre cards in BeginnerDeck
    var iter = deck.drawableByTag(.{ .manoeuvre = true });
