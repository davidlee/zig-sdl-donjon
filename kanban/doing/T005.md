...
- [ ] 5.5 Create `src/domain/apply/effects/positioning.zig`:
  - `ManoeuvreType` enum: advance, retreat, sidestep, hold
  - `ManoeuvreOutcome` enum: aggressor_succeeds, defender_succeeds, stalemate
  - `resolveManoeuvreConflict(aggressor, defender, moves, engagement, rng) -> Outcome`
  - `calculateManoeuvreScore(agent, move, engagement) -> f32` using speed + position + balance

- [ ] 5.6 Integrate contests into tick resolution:
  - Add `resolvePositioningContests(world)` in `apply/effects/positioning.zig`
  - Call after `executeManoeuvreEffects` in `world.processTick`
  - Add `getAgentFootwork(agent, enc, registry) -> ?ManoeuvreType` helper
  - Activate range propagation in `manoeuvre.zig` (line 86 TODO)

### Content: Multi-Opponent Manoeuvres

- [ ] 5.7 Add manoeuvre cards in `card_list.zig`:
  - `t_circle`: position bonus vs all (`.all_enemies`), `modify_position` effect
  - `t_disengage`: open range from all, `modify_range: { .steps = 1, .propagate = false }`
  - `t_pivot`: switch primary + position bonus (`.single`), sets `attention.primary`
- [ ] 5.7.1 Add `Effect.modify_position: f32` if needed
- [ ] 5.7.2 Add `Effect.set_primary_target` or rule expression for pivot
...
- [ ] 5.13 resolveManoeuvreConflict outcomes based on score differential
- [ ] 5.14 Standing still loses positioning contest
