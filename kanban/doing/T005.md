...
- [~] 5.6 Integrate contests into tick resolution: **IN PROGRESS**
  ...
  - [ ] Fix test `makeTestAgent` - needs weapon instance, see `agent.zig:307` for pattern
  - **Contest mechanics**: winner gets +1 bonus step + weapon reach floor
  - **Build passes** (`zig build` OK), tests need weapon instance in helper

  **Handover notes**:
  - Core impl done: `getAgentFootwork`, `resolvePositioningContests`, `applyContestOutcome`
  - Range propagation now works for multi-step moves (n-1 to other engagements)
  - Test helper at line ~297 needs weapon instance like `agent.zig:307` pattern
  - Files touched: `positioning.zig`, `manoeuvre.zig`, `world.zig`, `apply.zig`, `apply/mod.zig`
