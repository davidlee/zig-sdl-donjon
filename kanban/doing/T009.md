...
- Attacks have `.target = .all_enemies` - legacy from before targeting was wired up
- `validateMeleeReach` checks if *any* enemy is in range at selection time
- Range is treated as a card validity predicate, not a targeting constraint
...
1. **Update attack cards**: `.all_enemies` → `.single` (except true multi-target attacks)
3. **Remove selection-time range validation**: `validateMeleeReach` should not block card selection
4. **Add resolution-time range check**: When attack resolves, check range to actual target
5. **Handle whiff case**: Attack against out-of-range target misses/fizzles with feedback
