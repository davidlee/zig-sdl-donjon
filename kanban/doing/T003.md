...
- [ ] UI: target selection when playing `.single` cards
...
| `src/commands.zig` | Done | `play_card` now struct with optional target |
...
- [ ] UI: prompt for target when playing `.single` cards (future)
- [ ] UI: auto-assign target when single enemy (future)
