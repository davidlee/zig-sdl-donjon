...
| File | Status | Changes |
|------|--------|---------|
| `src/domain/combat.zig` | Done | `Play.target`, `Armament.getOffensiveMode`, `Encounter.*Const` methods, `Encounter.init` heap-allocates |
| `src/domain/world.zig` | Done | `encounter: ?*Encounter` (heap-allocated), removed `encounterPtr*()` helpers |
| `src/domain/apply.zig` | Done | `validateMeleeReach`, encounter param on `validateCardSelection`, updated ~15 encounter usages |
| `src/domain/tick.zig` | Done | Updated encounter access patterns |
| `src/domain/ai.zig` | Done | Updated to pass encounter directly (no helper) |
| `src/presentation/views/combat/view.zig` | Done | Updated to pass encounter directly (no helper) |
