...
- 2026-01-08: Implementation complete:
  - Added 8 new conditions to `Condition` enum in `damage.zig`:
    - Pain: `distracted` (>30%), `suffering` (>60%), `agonized` (>85%)
    - Trauma: `dazed` (>30%), `unsteady` (>50%), `trembling` (>70%), `reeling` (>90%)
    - Critical: `incapacitated` (>95% pain OR >95% trauma)
  - Added penalty entries to `condition_penalties` table
  - Added `resource_threshold` entries to `condition_definitions` using `.gt` comparator (pain/trauma accumulate)
  - `incapacitated` uses `.any` computation for OR logic across pain/trauma
  - Added display strings in `combat_log.zig`
  - Added 4 unit tests verifying threshold boundaries
  - All 203 tests pass, linted clean