...
- [x] 4.21 Verify `Command.play_card` handles footwork cards correctly
- [x] 4.22 Document UI contract for footwork placement (timeline positioning deferred)

### Tests

- [x] 4.23 `modify_range` adjusts engagement.range correctly
- [x] 4.24 Range clamped at boundaries (can't go below .clinch or above .far)
- [x] 4.25 Footwork overlaps weapon channel (no conflict)
- [x] 4.26 `getOverlayBonuses` aggregates from multiple overlapping manoeuvres
- [x] 4.27 Overlay damage bonus applied in attack resolution
- [x] 4.28 `.stationary` computed when no footwork in timeline
- [x] 4.29 `.stationary` NOT computed when footwork present
- [x] 4.30 Stationary defender: +10% hit chance for attacker
- [x] 4.31 Computed conditions (pressured, unbalanced) now affect resolution
...
- [x] Advance closes range by 1 step
- [x] Footwork overlaps weapon channel in timeline
- [x] Advance + thrust gives +10% damage
- [x] Sidestep + thrust gives +5% to_hit
- [x] Retreat + block gives +0.1 defense
- [x] Stationary defender = +10% hit chance for attacker
- [x] Computed conditions (pressured, unbalanced, weapon_bound) now affect resolution
- [x] Manoeuvres available in always_available pool
...
### Completed: Contracts (4.21-4.22)
7. **Command.play_card verification** (4.21):
   - `ChannelSet.conflicts()` only returns true for same-channel overlap
   - Footwork cards (`.footwork`) don't conflict with weapon cards (`.weapon`)
   - UI bypasses targeting mode for `.all_enemies` targeting (footwork cards)
   - `CommandHandler.playActionCard()` validates and plays correctly

8. **UI Contract for Footwork** (4.22):
   - **Pool**: Footwork cards in `always_available` pool (same as modifiers)
   - **Selection**: Use `Command.play_card` with card_id, target=null
   - **Targeting**: No target selection needed (`.all_enemies` resolves automatically)
   - **Channel overlap**: Footwork + weapon techniques coexist (different channels)
   - **Timeline**: Position determined by technique duration (same as weapon cards)
   - **Deferred**: Explicit timeline slot selection UI (future work, not needed for MVP)

### Completed: Tests (4.23-4.31)
9. **Range modification tests** (4.23-4.24) → `apply/effects/manoeuvre.zig`:
   - `adjustRange advances toward clinch`
   - `adjustRange retreats toward far`
   - `adjustRange clamps at clinch boundary`
   - `adjustRange clamps at far boundary`

10. **Channel overlap** (4.25): Already covered by existing tests in `cards.zig`:
    - `ChannelSet.conflicts detects overlap` (lines 463-464 verify footwork+weapon don't conflict)

11. **Overlay bonus tests** (4.26-4.27) → `resolution/context.zig`:
    - `getOverlayBonuses returns empty for agent with no timeline plays`
    - `getOverlayBonuses aggregates advance damage bonus`
    - `getOverlayBonuses aggregates sidestep to_hit bonus`
    - `getOverlayBonuses aggregates retreat defense bonus`
    - `getOverlayBonuses aggregates multiple overlapping manoeuvres`
    - Integration test in `resolution/outcome.zig`: `resolveOutcome applies overlay to_hit_bonus`

12. **Stationary condition tests** (4.28-4.30):
    - `hasFootworkInTimeline returns false when empty` → `combat/plays.zig`
    - `hasFootworkInTimeline returns true when footwork card present` → `combat/plays.zig`
    - `hasFootworkInTimeline returns false when only weapon card present` → `combat/plays.zig`
    - `CombatModifiers.forDefender applies stationary penalty` → `resolution/context.zig`
    - `CombatModifiers.forDefender no penalty when not stationary` → `resolution/context.zig`

13. **Computed conditions tests** (4.31) → `resolution/outcome.zig`:
    - `CombatModifiers.forAttacker reduces hit_chance when unbalanced`
    - `CombatModifiers.forDefender reduces defense_mult when pressured`

### Build Status
All tests pass. Build clean. Formatted.

---

**Created**: 2026-01-06
**Preflight**: 2026-01-06
**Last update**: 2026-01-06 (4.23-4.31 tests complete)
