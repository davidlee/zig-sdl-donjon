...
1. [x] Define `AgentTemplate` type (or confirm existing type suffices)
2. [x] Define `EncounterTemplate` type
3. [x] Create `src/data/personas.zig` with initial persona set
4. [~] Create `src/testing/fixtures.zig` with `AgentHandle`, `WorldHandle`, instantiation helpers
   - AgentHandle + agentFromTemplate: DONE
   - WorldHandle + worldFromEncounter: TODO
...
- 2026-01-08: Scaffolding complete:
  - Created `src/data/personas.zig` with:
    - `AgentTemplate`, `ArmamentTemplate`, `EncounterTemplate`, `DirectorKind` types
    - 8 agent personas: grunni_the_desperate, snik, ser_marcus, shield_bearer, sword_and_board, player_swordsman, spearman, knifeman
    - 3 weapon templates: thrown_rock, maybe_haunted, shortbow
    - 3 encounter templates: duel_at_sword_range, vs_shield_bearer, goblin_ambush
  - Created `src/testing/fixtures.zig` with:
    - `AgentHandle` struct with proper ownership/deinit
    - `agentFromTemplate()` - instantiates agent from persona template
    - 6 passing tests for fixture functionality
  - All builds pass: `just check` green

### Key files:
- `src/data/personas.zig` - template types and persona data
- `src/testing/fixtures.zig` - instantiation helpers
- `src/testing/mod.zig` - re-exports fixtures

### 2026-01-08: T019 COMPLETE
- Deleted 4 duplicate `rulePredicatesSatisfied` tests from validation.zig (already passing in apply.zig)
- Deleted 5 skipped tests needing Encounter/World setup → documented in T020 for integration testing
- Updated `style_conventions` memory with fixture usage patterns
- `worldFromEncounter()` deferred to T020 integration harness (not needed for unit test fixtures)
