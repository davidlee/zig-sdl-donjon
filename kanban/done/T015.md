...
- [x] condition_applied event (existing) used for dud injection (no new event needed)
- [x] Wince injected when .distracted gained
- [x] Tremor blocks .precision cards while in hand
- [x] Playing dud exhausts it (via `.cost.exhausts = true`)
- [x] Involuntary cards cannot be withdrawn (Focus un-commit)
...
1. **Dud card templates** (`card_list.zig:810-907`)
   - `dud_wince`: Time cost (0.6), no blocking - distracted causes fumbling
   - `dud_tremor`: Blocks precision cards - trembling hands can't aim
   - `dud_retch`: Blocks finesse cards - nausea disrupts dexterity
   - `dud_stagger`: Blocks manoeuvre cards - unsteady footing
   - `dud_blackout`: Blocks offensive cards - vision impaired

2. **Condition→card mapping** (`card_list.zig:897-907`)
   - `condition_dud_cards: EnumArray(Condition, ?*const Template)`
   - Maps: distracted→wince, trembling→tremor, nauseous→retch, unsteady→stagger, reeling→blackout

3. **Injection in event processor** (`event_processor.zig:201-230`)
   - `injectDudCardIfMapped()` handles `condition_applied` events
   - Creates card instance, adds to agent's hand, emits `card_moved` event
   - Uses existing `condition_applied` event (no new event type needed)

4. **Zone.limbo** (`cards.zig:50`)
   - Added virtual zone for cards created from nowhere (dud card injection source)

5. **Involuntary withdrawal restriction** (`validation.zig:114-126`)
   - Extended `canWithdrawPlay()` to check involuntary tag
   - Involuntary cards cannot be un-committed with Focus

### Design Decisions

- Used `.kind = .action` with `.involuntary` tag (not new `.status` kind)
- Placed templates in `card_list.zig` alongside other templates
- Included involuntary withdrawal restriction (not deferred)

### Tests Added

- `canWithdrawPlay returns false for involuntary cards`
- `canWithdrawPlay returns true for non-involuntary cards with no modifiers`
