...
- [x] **Severity mapping & volume thresholds (critical review §2.3 / T039, complete 2026-01-10).** Dual severity model: `severityFromVolume()` (energy-driven) and `severityFromDepth()` (geometry-driven). Non-structural layers cap at `.disabled`; structural layers require volume for `.missing`. `is_structural` flag added to materials schema. Small parts (area < 30 cm²) have reduced severing thresholds.
...
- **Contact geometry derived from damage.Kind (T039).** Geometry as a scalar can't distinguish point (needle) from plane (scimitar), but severing logic doesn't need a new field: `checkSevering` switches on `damage.Kind` where slash=edge (easier sever), pierce=point (harder), bludgeon=blunt (harder). The 3-axis model stays clean; `Kind` acts as implicit contact-shape discriminator.
...
- [x] **Test coverage gaps (critical review §3)(T040, complete 2026-01-10).** Data-driven combat tests using CUE-defined scenarios with generated weapons/armour. Deterministic RNG (T041) enables reproducible results.
- [x] **Severity vs. volume modelling (T039, complete 2026-01-10).** `.missing` requires structural layer AND volume damage (energy-driven). Depth alone (geometry) caps at `.broken`. Non-structural layers cap at `.disabled`. Severing requires both penetration depth AND volume loss.
